...
- **趨勢判斷**：
  - 多頭：MA10 斜率 > 0、MA20 斜率 > 0，且 MA10 > MA20
  - 空頭：MA10 斜率 < 0、MA20 斜率 < 0，且 MA10 < MA20
- **進場條件**：
  - 多頭進場：多頭趨勢成立 + 前一根 K 棒觸及 MA10 或 MA20，且當前收盤 > min(前一根 Open, 前一根 Close)，同時收盤站上 MA10/MA20
  - 空頭進場：空頭趨勢成立 + 前一根 K 棒觸及 MA10 或 MA20，且當前收盤 < min(前一根 Open, 前一根 Close)，同時收盤跌破 MA10/MA20
- **停損 / 出場條件**：
  - 多頭停損：當前收盤 < min(前一根 Open, 前一根 Close)
  - 空頭停損：當前收盤 > max(前一根 Open, 前一根 Close)
...
- **績效統計**：在「📋 交易紀錄」中顯示：總交易數、做多/做空筆數、總損益、獲利筆數、虧損筆數、勝率（pnl>0 的比例）
- **圖表互動**：主圖開啟底部 range slider，可拖曳查看更早 K 棒，搭配「顯示 K 棒數量」拉大筆數即可向左滑動看歷史
- **預設回測**：
  - 未登入 Shioaji：預設使用「日盤 + 日 K(1d) + 100 根」
  - 已登入 Shioaji：預設使用「日盤 + 5 分 K(5m) + 150 根」
...
**最後更新**: 2026-02-22  
**數據範圍**: 2026-01-01 ~ 2026-02-09（含完整日盤/夜盤，後續持續回填中）  
**數據庫記錄**: 約 24k+ 筆 1 分 K（會隨回填成長）  
**驗證狀態**: ✅ 主要交易日與券商 APP 逐日交叉驗證通過
